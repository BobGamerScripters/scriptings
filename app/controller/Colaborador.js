...
export default class Colaborador {
  ...
  static async find(data = {}) {
    try {
      ...
      const [{ count: total }] = await connection(Colaborador.table).count(
        "id as count",
      );
        ...
        if (search) {
          query.where(function () {
            for (const col of Enterprise.#searchable) {
              this.orWhereRaw(`CAST("${col}" AS TEXT) ILIKE ?`, [
                `%${search}%`,
              ]);
            }
          });
      ...
      const filteredQ = connection(Colaborador.table).count("id as count");
      applySearch(filteredQ);
      const [{ count: filtered }] = await filteredQ;

      const orderColumn = Colaborador.#columns[column] || "id";
      const orderDir = orderType === "desc" ? "desc" : "asc";

      const dataQ = connection(Colaborador.table).select("*");
      applySearch(dataQ);
      ...
      console.error("Erro no find enterprises:", err);
      return {
        draw: 1,
        recordsTotal: 0,
        recordsFiltered: 0,
  ...
  static async findById(id) {
    ...
    try {
      const row = await connection(Colaborador.table).where({ id }).first();
      return row || null;
    } catch (err) {
  ...
  static async insert(data) {
    ...
    try {
      const clean = Colaborador.#sanitize(data);

      const [result] = await connection(Colaborador.table)
        .insert(clean)
        .returning("*");
  ...
  static async update(id, data) {
    ...
    try {
      ...
      if (!result) {
        return { status: false, msg: "Colaborador não encontrada", data: [] };
      }
  ...
  static async delete(id) {
    ...
    try {
      await connection(Colaborador.table).where({ id }).del();
